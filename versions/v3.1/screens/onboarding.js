...
/**
 * Has this theme x script actually been rendered?
 *
 * data/onboarding-films.js is generated by the render and lists what exists.
 ...
 */
function onbFilmAvailable() {
  if (onbFilmBroken) return false;
  if (typeof ONBOARDING_FILMS === "undefined") return false;
  const list = ONBOARDING_FILMS[onbFilmTheme()];
  return !!(list && list.indexOf(onbVideoScriptId()) !== -1);
}

/** Which render this tester should see: their theme, their primary goal. */
function onbFilmSrc() {
  return ONB_FILM_DIR + "/" + onbFilmTheme() + "/" + onbVideoScriptId() + ".mp4";
}
...
function onbVideoStage(v) {
  ...
  if (onbFilmAvailable()) {
    // `data-theme` keys the element to the render it is showing, so a theme
    // switch mid-film replaces the node (and its src) while an ordinary repaint
    // leaves it alone. Muted + playsinline because the picture carries no audio
    // and an unmuted autoplay-capable element is refused by some browsers.
    return `
    <div class="lp-stage lp-stage-video onb-video-stage">
      <video class="onb-film" id="onb-film" muted playsinline preload="auto"
             data-theme="${h(onbFilmTheme())}"
